  ...
  return (
    <Div>
      ...
      post.map(item=>(
        <List key={item.head}>
          <Icon>
           {item.icon==="text"?<div className='flex items-center justify-center'>{item.text}
           </div>:<Image
          ...
          <div className='flex flex-col w-full truncate'>
           <DivContent>
            ...
            <Paragraph>It is <span className='text-black font-medium dark:text-white '>
            {item.name}
            </span> <span>`s &nbsp;</span></Paragraph>:<Paragraph>{item.name} &nbsp;</Paragraph>}
            {<p>{item.head} &nbsp;</p>}
            {<p className='text-black font-medium dark:text-white'>{item.text==="JL"?<span>&quot;{item.lew}&quot;</span>:""}
            <span className='font-normal truncate'>{item.post1}</span></p>}
            {<Paragraph>{item.name==="Dunn Slane"?<span className='truncate'>&quot;{item.well}&quot;</span>:""}</Paragraph>}
            ...
            {<Paragraph className='truncate'>{item.haskel} 
            <span className='font-normal'> {item.from} </span>{item.open}
            <span className='font-normal'> {item.to} </span>{item.closed}</Paragraph>}
            {<Paragraph className='truncate'>{item.lorry}</Paragraph>}
            {<Paragraph className='truncate'>{item.sunny}</Paragraph>}
            {<Paragraph> {item.dollar}
            <span className='font-normal truncate'> {item.geofry}</span></Paragraph>}
            ...
            {item.id==="id"?<div className='justify-end'>
            {item.id==="id"?
